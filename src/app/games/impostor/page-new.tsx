...
import { ANIME_CHARACTERS } from '@/lib/anime-data-clean';
import { AnimeCharacter } from '@/lib/anime-data-clean';
...
const IMPOSTOR_THEMES = [
  {
    ...
    getGameData: () => {
      const strawHats = ANIME_CHARACTERS.filter(char => char.affiliation === 'Straw Hat Pirates');
      const correctChars = strawHats.slice(0, 4);
      const nonStrawHats = ANIME_CHARACTERS.filter(char => char.affiliation !== 'Straw Hat Pirates');
      ...
      const impostorIndex = shuffled.findIndex(char => char.team !== 'Straw Hat Pirates');
      
      return {
        theme: 'Straw Hat Pirates',
        characters: shuffled,
        impostorIndex,
        explanation: `${impostorChar.name} is not a Straw Hat Pirate - they are part of ${impostorChar.team}`
      };
    }
  },
  {
    ...
    getGameData: () => {
      const marines = ANIME_CHARACTERS.filter(char => char.team === 'Marines');
      const correctChars = marines.slice(0, 4);
      const nonMarines = ANIME_CHARACTERS.filter(char => char.team !== 'Marines');
      const impostorChar = nonMarines[Math.floor(Math.random() * nonMarines.length)];
      
      const allChars = [...correctChars, impostorChar];
      const shuffled = allChars.sort(() => Math.random() - 0.5);
      const impostorIndex = shuffled.findIndex(char => char.team !== 'Marines');
      
      return {
        theme: 'Marines',
        characters: shuffled,
        impostorIndex,
        explanation: `${impostorChar.name} is not a Marine - they are part of ${impostorChar.team}`
      };
    }
  },
  {
  ...
  },
  {
    ...
    description: 'Characters with power level 90 or above',
    getGameData: () => {
      const highPowerChars = ANIME_CHARACTERS.filter(char => char.power >= 90);
      const lowPowerChars = ANIME_CHARACTERS.filter(char => char.power < 80);
      
      const correctChars = highPowerChars.slice(0, 4);
      const impostorChar = lowPowerChars[Math.floor(Math.random() * lowPowerChars.length)];
      
      const allChars = [...correctChars, impostorChar];
      const shuffled = allChars.sort(() => Math.random() - 0.5);
      const impostorIndex = shuffled.findIndex(char => char.power < 80);
      
      return {
        theme: 'High Power Characters (90+)',
        characters: shuffled,
        impostorIndex,
        explanation: `${impostorChar.name} has low power (${impostorChar.power}), not high power (90+)`
      };
    }
  },
  {
    name: 'Four Emperors Era',
    description: 'Characters from Emperor crews',
    getGameData: () => {
      const emperorTeams = ['Red Hair Pirates', 'Whitebeard Pirates', 'Blackbeard Pirates', 'Beast Pirates', 'Big Mom Pirates'];
      const emperorChars = ANIME_CHARACTERS.filter(char => emperorTeams.includes(char.team));
      const nonEmperorChars = ANIME_CHARACTERS.filter(char => !emperorTeams.includes(char.team));
      
      const correctChars = emperorChars.slice(0, 4);
      const impostorChar = nonEmperorChars[Math.floor(Math.random() * nonEmperorChars.length)];
      
      const allChars = [...correctChars, impostorChar];
      const shuffled = allChars.sort(() => Math.random() - 0.5);
      const impostorIndex = shuffled.findIndex(char => !emperorTeams.includes(char.team));
      
      return {
        theme: 'Four Emperors Era',
        characters: shuffled,
        impostorIndex,
        explanation: `${impostorChar.name} is not from an Emperor crew - they are from ${impostorChar.team}`
      };
    }
  },
  {
    name: 'Wano Arc',
    description: 'Characters from the Wano arc',
    getGameData: () => {
      const wanoTeams = ['Wano', 'Nine Red Scabbards', 'Beast Pirates'];
      const wanoChars = ANIME_CHARACTERS.filter(char => 
        wanoTeams.includes(char.team) || char.team.includes('Wano')
      );
      const nonWanoChars = ANIME_CHARACTERS.filter(char => 
        !wanoTeams.includes(char.team) && !char.team.includes('Wano')
      );
      
      const correctChars = wanoChars.slice(0, 4);
      const impostorChar = nonWanoChars[Math.floor(Math.random() * nonWanoChars.length)];
      
      const allChars = [...correctChars, impostorChar];
      const shuffled = allChars.sort(() => Math.random() - 0.5);
      const impostorIndex = shuffled.findIndex(char => 
        !wanoTeams.includes(char.team) && !char.team.includes('Wano')
      );
      
      return {
        theme: 'Wano Arc Characters',
        characters: shuffled,
        impostorIndex,
        explanation: `${impostorChar.name} is not from the Wano arc - they are from ${impostorChar.team}`
      };
    }
  }
];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-yellow-400 mb-2">One Piece Impostor</h1>
            <p className="text-purple-200">Find the character that doesn't belong!</p>
          </div>
